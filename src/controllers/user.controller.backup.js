...
const loginUser = asyncHandler( async (req, res) => {
   ...
   const loggedInUser = await User.findById(user._id).select("-password -refreshToken") // find user in db, return it but do not return password and refresh_token   
   ...
   const user = await User.findByIdAndUpdate(
      req.user?._id,
      {
         $set: {
            fullName,
            email: email 
            //both are true fullName: fullName, email
         }
      },
      {new: true} //it return the updated user information
   ).select("-password")
